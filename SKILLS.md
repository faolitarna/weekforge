...
Shared in `.agents/skills/`, project-specific in `.agents-local/skills/`.
...
### Specs Management
- **Path**: `.agents-local/skills/specs-management/SKILL.md` (weekforge override — uses `specs/` paths)
- **Shared fallback**: `.agents/skills/specs-management/SKILL.md` (generic, uses `{SPECS_DIR}`)
...
### Prompt Review
- **Path**: `.agents/skills/prompt-review/SKILL.md` (shared)
...
Shared: create `agents/<name>/SKILL.md` in `~/Projects/dev-agents`, commit there.
Project-specific: create `.agents-local/skills/<name>/SKILL.md` here, tracked in this repo.
