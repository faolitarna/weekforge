...
**Plan Revision Loop.** The graph edge from HITL plan review routes back to the planner node when the user provides feedback. The loop repeats until the user explicitly approves. Only then does the graph transition to the generation phase.
